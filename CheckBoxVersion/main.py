...
def main(page: ft.Page):
    ...
    async def transcribir(e):
        ...
        try:
            if file_name != "Cancelado" and file_name is not None:
                whisper_python_path = os.path.abspath("src/whisperp.py")
                commandtxt.value = f'python "{whisper_python_path}" "{file_name}" {selected_model} {selected_device} {c1.value}'
    ...
    c1 = ft.Checkbox(label="Tiempos", value=False)
